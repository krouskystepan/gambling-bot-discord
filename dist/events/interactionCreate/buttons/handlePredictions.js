    ...
    try {
        ...
        if (targetPrediction.status !== 'active') {
            return await interaction.reply({
                embeds: [
                    createInfoEmbed('Invalid Input - Not Active', 'This prediction is not active.')
                ],
        ...
        if (!targetPrediction.choices || targetPrediction.choices.length === 0) {
            return await interaction.reply({
                embeds: [
                    createInfoEmbed('Invalid Input - No Choices', 'This prediction has no choices available.')
                ],
                flags: MessageFlags.Ephemeral
            });
        }
        ...
        if (isNaN(parsedBetAmount) || parsedBetAmount <= 0) {
            return modalInteraction.editReply({
                embeds: [
                    createInfoEmbed('Invalid Input - Non-positive number', 'Please enter a valid positive number.')
                ]
            });
        }
        ...
        if (casinoSettings.prediction.maxBet > 0 &&
            ...
            return modalInteraction.editReply({
                embeds: [
                    createInfoEmbed('Invalid Input - Above Maximum Bet', `The maximum bet per choice is **$${formatNumberToReadableString(casinoSettings.prediction.maxBet)}**. You already have **$${formatNumberToReadableString(userChoiceTotal)}** on **${choiceName}**.`)
                ]
            });
        }
        if (casinoSettings.prediction.minBet > 0 &&
            ...
            return modalInteraction.editReply({
                embeds: [
                    createInfoEmbed('Invalid Input - Below Minimum Bet', `The minimum bet is **$${formatNumberToReadableString(casinoSettings.prediction.minBet)}**.`)
                ]
            });
        }
        ...
        await modalInteraction.editReply({
            embeds: [
                createSuccessEmbed('Bet Placed Successfully', `You placed **$${formatNumberToReadableString(parsedBetAmount)}** on **${choiceName}**`)
            ]
        });
    }
    catch (error) {
