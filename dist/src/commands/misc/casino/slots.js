...
async function run({ interaction }) {
    try {
        ...
        if (casinoConfig_1.SLOT_MAX_BET > 0 && parsedBetAmount > casinoConfig_1.SLOT_MAX_BET) {
            return interaction.reply({
                embeds: [
                    (0, createEmbed_1.createInfoEmbed)('Invalid Input - Above Maximum Bet', `The maximum bet is **$${(0, utils_1.formatNumberToReadableString)(casinoConfig_1.SLOT_MAX_BET)}**.`),
                ],
                flags: discord_js_1.MessageFlags.Ephemeral,
            });
        }
        ...
        for (let i = 0; i < spins; i++) {
            const resultString = (0, casinoHelpers_1.spinSlot)();
            const winnings = (casinoConfig_1.SLOT_MULTIPLIERS[resultString] || 0) * parsedBetAmount;
