...
async function run({ interaction }) {
    try {
        ...
        if (casinoConfig_1.COINFLIP_MAX_BET > 0 && parsedBetAmount > casinoConfig_1.COINFLIP_MAX_BET) {
            return interaction.reply({
                embeds: [
                    (0, createEmbed_1.createInfoEmbed)('Invalid Input - Above Maximum Bet', `The maximum bet is **$${(0, utils_1.formatNumberToReadableString)(casinoConfig_1.COINFLIP_MAX_BET)}**.`),
                ],
                flags: discord_js_1.MessageFlags.Ephemeral,
            });
        }
        ...
        if (user.balance < totalBet) {
            return interaction.reply({
                embeds: [
                    (0, createEmbed_1.createInfoEmbed)('Insufficient Funds', `You don't have enough money to place this bet for ${flips} flips (you need **$${(0, utils_1.formatNumberToReadableString)(totalBet)}**).\nYour current balance is **$${(0, utils_1.formatNumberToReadableString)(user.balance)}**.`),
                ],
                flags: discord_js_1.MessageFlags.Ephemeral,
            });
        }
        ...
        for (let i = 0; i < flips; i++) {
            ...
            const winnings = win ? parsedBetAmount * casinoConfig_1.COINFLIP_WIN_MULTIPLIER : 0;
            results.push(`**${flipResult === 'heads' ? 'H 👑' : 'T 🐍'}** | ${win ? '🎉' : '❌'} | ${win
                ? `**+$${(0, utils_1.formatNumberToReadableString)(winnings)}**`
                : `**-$${(0, utils_1.formatNumberToReadableString)(parsedBetAmount)}**`}`);
        ...
        return interaction.reply({
            embeds: [
                (0, createEmbed_1.createBetEmbed)(isWin
                    ...
                    `🪙 **Flip Results:**\n${results.join('\n')}\n\n` +
                    `💰 Total Winnings: ${isWin ? '🟢' : isLoss ? '🔴' : '🟡'} **$${(0, utils_1.formatNumberToReadableString)(totalWinnings)}**\n` +
                    (showBalance
