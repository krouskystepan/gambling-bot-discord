...
async function run({ interaction }) {
    try {
        ...
        if (spins > casinoConfig_1.MAX_SIMULATE_SPINS) {
            return interaction.editReply({
                content: `The maximum number of spins is ${(0, utils_1.formatNumberToReadableString)(casinoConfig_1.MAX_SIMULATE_SPINS)}.`,
            });
        }
        ...
        for (let i = 1; i <= spins; i++) {
            ...
            let winnings = 0;
            if (casinoConfig_1.SLOT_MULTIPLIERS[resultString]) {
                winnings = bet * casinoConfig_1.SLOT_MULTIPLIERS[resultString];
                wins++;
        ...
        const winDetails = Object.entries(winCounts)
            ...
            .join('\n');
        const multipliersDetails = Object.entries(casinoConfig_1.SLOT_MULTIPLIERS)
            .map(([symbol, multiplier]) => `${symbol}: **${multiplier}**x`)
            .join('\n');
        const symbolWeightsDetails = Object.entries(casinoConfig_1.SYMBOL_WEIGHTS)
