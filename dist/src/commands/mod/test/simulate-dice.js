...
async function run({ interaction }) {
    try {
        ...
        if (rolls > casinoConfig_1.MAX_SIMULATE_ROLLS) {
            return interaction.editReply({
                content: `The maximum number of rolls is **${(0, utils_1.formatNumberToReadableString)(casinoConfig_1.MAX_SIMULATE_ROLLS)}**.`,
            });
        }
        ...
        for (let i = 1; i <= rolls; i++) {
            ...
            let winnings = 0;
            if (diceRoll === 6) {
                winnings = bet * casinoConfig_1.DICE_WIN_MULTIPLIER;
                wins++;
        ...
        const multipliersDetails = `**${casinoConfig_1.DICE_WIN_MULTIPLIER}x**`;
