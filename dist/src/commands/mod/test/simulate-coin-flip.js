...
async function run({ interaction }) {
    try {
        ...
        if (flips > casinoConfig_1.MAX_SIMULATE_FLIPS) {
            return interaction.editReply({
                content: `The maximum number of flips is **${(0, utils_1.formatNumberToReadableString)(casinoConfig_1.MAX_SIMULATE_FLIPS)}**.`,
            });
        }
        ...
        for (let i = 1; i <= flips; i++) {
            ...
            let winnings = 0;
            if (coinFlip === 'heads') {
                winnings = bet * casinoConfig_1.COINFLIP_WIN_MULTIPLIER;
                wins++;
        ...
        const multipliersDetails = `**${casinoConfig_1.COINFLIP_WIN_MULTIPLIER}x**`;
