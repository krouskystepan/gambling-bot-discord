    ...
    try {
        ...
        if (!guildConfiguration) {
            return interaction.reply({
                embeds: [
                    createErrorEmbed('Guild Not Configured', 'This guild has not been configured yet. Please set it up first.')
                ],
        ...
        if (subcommand === 'create-room') {
            ...
            if (targetedUser.bot) {
                return interaction.reply({
                    embeds: [
                        createInfoEmbed('Invalid Input - Bot user', 'You cannot create a VIP room for bots.')
                    ],
                    flags: MessageFlags.Ephemeral
                });
            }
            ...
            if (existingVip) {
                return interaction.reply({
                    embeds: [
                        createErrorEmbed('VIP Already Active', `User <@${targetedUser.id}> already has a VIP channel: <#${existingVip.channelId}>.`)
                    ],
                    flags: MessageFlags.Ephemeral
                });
            }
            if (!/^(\d+[dw])+$/i.test(durationInput)) {
                return interaction.reply({
                    embeds: [
                        createInfoEmbed('Invalid Input - Invalid Duration', 'Format is invalid. Use whole numbers only, e.g., 1d, 2w.')
                    ],
                    flags: MessageFlags.Ephemeral
                });
            }
            ...
            await member.roles.add(vipRoleOwnerId, `VIP created by admin ${interaction.user}`);
            await createVip({
                ownerId: targetedUser.id,
                guildId: interaction.guildId,
            ...
            return interaction.reply({
                embeds: [
                    createSuccessEmbed('VIP Room Created', `VIP room <#${channel.id}> has been created for <@${targetedUser.id}>.\n` +
        ...
        if (subcommand === 'remove-room') {
            ...
            if (targetedUser.bot) {
                return interaction.reply({
                    embeds: [
                        createInfoEmbed('Invalid Input - Bot user', 'You cannot remove a VIP room for bot.')
                    ],
                    flags: MessageFlags.Ephemeral
                });
            }
            ...
            if (!existingVip) {
                return interaction.reply({
                    embeds: [
                        createErrorEmbed('VIP Not Active', `User <@${targetedUser.id}> does not currently have an active VIP room.`)
                    ],
                    flags: MessageFlags.Ephemeral
                });
            }
            ...
            if (channel && channel.isTextBased()) {
                ...
                await channel
                    ...
                    embeds: [
                        createInfoEmbed('VIP Channel Removed', '⏰ Your VIP access has been removed. You no longer have permission to send messages in this channel.\n\n' +
            ...
            return interaction.reply({
                embeds: [
                    createSuccessEmbed('VIP Removed', `The VIP of <@${targetedUser.id}> has been removed.\n` +
        ...
        if (subcommand === 'extend-room') {
            ...
            if (targetedUser.bot) {
                return interaction.reply({
                    embeds: [
                        createInfoEmbed('Invalid Input - Bot user', 'You cannot extend VIP for bots.')
                    ],
                    flags: MessageFlags.Ephemeral
                });
            }
            ...
            if (!updatedVip) {
                return interaction.reply({
                    embeds: [
                        createErrorEmbed('VIP Not Found', `User <@${targetedUser.id}> does not currently have an active VIP room.`)
                    ],
                    flags: MessageFlags.Ephemeral
                });
            }
            ...
            if (vipChannel?.isTextBased()) {
                const extendMsg = await vipChannel.send({
                    content: `<@${targetedUser.id}>`,
                    embeds: [
                        createSuccessEmbed('VIP Channel Extended', `Your VIP now expires on <t:${Math.floor(updatedVip.expiresAt.getTime() / 1000)}:f>.`)
                    ]
                });
                await extendMsg.pin();
            }
            return interaction.reply({
                embeds: [
                    createSuccessEmbed('VIP Extended', `The VIP of <@${targetedUser.id}> has been extended by **${durationSeconds / 86400} day(s)**.\nNow expires on: <t:${Math.floor(updatedVip.expiresAt.getTime() / 1000)}:f>`)
                ],
                flags: MessageFlags.Ephemeral
            });
        }
        if (subcommand === 'add-member') {
            ...
            const bypass = interaction.options.getBoolean('bypass-member-limit') ?? false;
            if (userToAdd.bot) {
                return interaction.reply({
                    embeds: [
                        createInfoEmbed('Invalid Input - Bot User', 'You cannot add bot users to VIP rooms.')
                    ],
                    flags: MessageFlags.Ephemeral
                });
            }
            ...
            if (!vipRoom) {
                return interaction.reply({
                    embeds: [
                        createErrorEmbed('VIP Not Found', `<@${ownerUser.id}> does not have an active VIP room.`)
                    ],
                    flags: MessageFlags.Ephemeral
                });
            }
            if (userToAdd.id === ownerUser.id) {
                return interaction.reply({
                    embeds: [
                        createInfoEmbed('Not Allowed', 'The owner is already part of the VIP room.')
                    ],
                    flags: MessageFlags.Ephemeral
                });
            }
            if (vipRoom.memberIds.includes(userToAdd.id)) {
                return interaction.reply({
                    embeds: [
                        createInfoEmbed('Already Added', `${userToAdd} is already a member of this VIP room.`)
                    ],
                    flags: MessageFlags.Ephemeral
                });
            }
            if (!bypass &&
                ...
                return interaction.reply({
                    embeds: [
                        createErrorEmbed('VIP Full', `This VIP room is full. Max members allowed: **${guildConfiguration.vipSettings.maxMembers}**`)
                    ],
                    flags: MessageFlags.Ephemeral
                });
            }
            ...
            return interaction.reply({
                embeds: [
                    createSuccessEmbed('Member Added', `${userToAdd} has been added to <@${ownerUser.id}>'s VIP room.` +
                        (bypass
