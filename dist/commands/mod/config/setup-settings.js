    ...
    try {
        ...
        if (subCommand === 'edit') {
            ...
            if (!guildConfiguration.casinoSettings[game]) {
                return interaction.reply({
                    embeds: [
                        createInfoEmbed('Invalid Game', 'The game you provided is not valid.\nPlease make sure you enter a valid game name.')
                    ],
            ...
            if (value.endsWith('%')) {
                ...
                if (valueAsNumber < 0 || valueAsNumber > 1) {
                    return interaction.reply({
                        embeds: [
                            createInfoEmbed('Invalid Input - Out of Range', 'The number you provided must be between 10% and 90%.\nPlease enter a positive value.')
                        ],
                        flags: MessageFlags.Ephemeral
                    });
                }
                ...
                return await interaction.reply({
                    embeds: [
                        createSuccessEmbed('Casino Settings Updated', `The setting **${readableGameValueNames.find((value) => value.value === key)
                            ?.name}** for **${game.toUpperCase()}** has been updated to **${formatNumberToPercentage(valueAsNumber)}**.`)
                    ],
                    flags: MessageFlags.Ephemeral
                });
            }
            ...
            if (value.toLocaleLowerCase().endsWith('k') ||
                ...
                const parsedValue = parseReadableStringToNumber(value.startsWith('*') ? value.slice(1) : value);
                ...
                if (key === 'minBet' &&
                    ...
                    return interaction.reply({
                        embeds: [
                            createInfoEmbed('Invalid Input - Above Maximum Bet', `The minimum bet cannot be greater than the maximum bet of **$${formatNumberToReadableString(guildConfiguration.casinoSettings[game].maxBet)}**.`)
                        ],
                        flags: MessageFlags.Ephemeral
                    });
                }
                ...
                return await interaction.reply({
                    embeds: [
                        createSuccessEmbed('Casino Settings Updated', `The setting ${key} for ${game} has been updated to **${formatNumberToReadableString(parsedValue)}**.`)
                    ],
                    flags: MessageFlags.Ephemeral
                });
            }
            if (value.includes(':')) {
                ...
                if (guildConfiguration.casinoSettings[game][key][objKey] === undefined) {
                    return interaction.reply({
                        embeds: [
                            createInfoEmbed('Invalid Input - Not a valid object key', 'The object key you entered is not valid.\nPlease make sure you enter a valid object key.')
                        ],
                        flags: MessageFlags.Ephemeral
                    });
                }
            ...
            interaction.reply({
                embeds: [
                    createInfoEmbed('Casino Settings', [
        ...
        if (subCommand === 'reset') {
            ...
            if (game === 'all') {
                ...
                return await interaction.reply({
                    embeds: [
                        createSuccessEmbed('Casino Settings Reset', 'All casino settings have been reset to default values.')
                    ],
                    flags: MessageFlags.Ephemeral
                });
            }
            if (game) {
                ...
                return await interaction.reply({
                    embeds: [
                        createSuccessEmbed('Casino Settings Reset', `The casino settings for **${game.toUpperCase()}** have been reset to default values.`)
                    ],
                    flags: MessageFlags.Ephemeral
                });
            }
