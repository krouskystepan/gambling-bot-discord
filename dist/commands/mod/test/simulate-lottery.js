...
async function run({ interaction }) {
    try {
        ...
        if (entries > defaultConfig_1.LOTTERY_MAX_SIMULATE_ENTRIES) {
            return interaction.editReply({
                content: `The maximum number of entries is ${(0, utils_1.formatNumberToReadableString)(defaultConfig_1.LOTTERY_MAX_SIMULATE_ENTRIES)}.`,
            });
        }
        ...
        const winDetails = Object.entries(winCounts)
            ...
            .join('\n');
        const multipliersDetails = Array.from({ length: 6 }, (_, i) => `${i}: **${settings.lottery.winMultipliers[i]}**x`).join('\n');
