    ...
    try {
        ...
        if (entries > GOLDEN_JACKPOT_MAX_SIMULATE_ENTRIES) {
            return interaction.editReply({
                content: `The maximum number of entries is **${formatNumberToReadableString(GOLDEN_JACKPOT_MAX_SIMULATE_ENTRIES)}**.`
            });
        }
        ...
        const embed = createBetEmbed(`🤑 GoldenJackpot Simulation - ${formatNumberToReadableString(entries)} entries`, profitOrLoss >= 0 ? 'Green' : 'Red', `Total bet: **$${formatNumberToReadableString(totalBet)}**\n` +
