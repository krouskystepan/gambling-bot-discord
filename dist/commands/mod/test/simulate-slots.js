...
export async function run({ interaction }) {
    try {
        ...
        const spins = parseReadableStringToNumber(interaction.options.getString('spins', true));
        if (spins > SLOT_MAX_SIMULATE_SPINS) {
            return interaction.editReply({
                content: `The maximum number of spins is ${formatNumberToReadableString(SLOT_MAX_SIMULATE_SPINS)}.`
            });
        }
        ...
        await interaction.editReply(`Simulating **${formatNumberToReadableString(spins)}** spins with a bet of **$${formatNumberToReadableString(bet)}**. Please wait...`);
        ...
        const embed = createBetEmbed(`🎰 Slot Simulation - ${formatNumberToReadableString(spins)} spins`, profitOrLoss >= 0 ? 'Green' : 'Red', `Total bet: **$${formatNumberToReadableString(totalBet)}**\n` +
