...
export async function run({ interaction }) {
    try {
        ...
        const rolls = parseReadableStringToNumber(interaction.options.getString('rolls', true));
        if (rolls > DICE_MAX_SIMULATE_ROLLS) {
            return interaction.editReply({
                content: `The maximum number of rolls is **${formatNumberToReadableString(DICE_MAX_SIMULATE_ROLLS)}**.`
            });
        }
        ...
        await interaction.editReply(`Simulating **${formatNumberToReadableString(rolls)}** rolls with a bet of **$${formatNumberToReadableString(bet)}**. Please wait...`);
        ...
        const embed = createBetEmbed(`🎲 Dice Simulation - ${formatNumberToReadableString(rolls)} rolls`, profitOrLoss >= 0 ? 'Green' : 'Red', `Total bet: **$${formatNumberToReadableString(totalBet)}**\n` +
