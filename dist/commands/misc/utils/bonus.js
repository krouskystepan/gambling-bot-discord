...
export const options = { deleted: false };
    ...
    try {
        ...
        if (!guildConfig || !guildConfig.bonusSettings) {
            return interaction.reply({
                embeds: [
                    createErrorEmbed('Error - Bonus not configured', 'Daily bonus is not configured for this server.')
                ],
        ...
        if (subcommand === 'claim') {
            ...
            if (!canClaim) {
                ...
                return interaction.reply({
                    embeds: [
                        createInfoEmbed('Daily Bonus Already Claimed', `Come back at **<t:${Math.floor(nextClaim.getTime() / 1000)}:f> / <t:${Math.floor(nextClaim.getTime() / 1000)}:R>**`)
                    ],
                    flags: MessageFlags.Ephemeral
                });
            }
            ...
            if (!updatedUser) {
                return interaction.reply({
                    embeds: [
                        createInfoEmbed('Daily Bonus Already Claimed', 'You already claimed your daily bonus.')
                    ],
                    flags: MessageFlags.Ephemeral
                });
            }
            ...
            const embed = new EmbedBuilder()
                ...
                .setDescription(`You claimed your daily bonus and received **$${formatNumberToReadableString(reward)}** coins!`)
                .addFields({
                name: '🔥 Current Streak',
