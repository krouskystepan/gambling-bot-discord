...
export const blackjackIdleCloseJob = async (client: Client<true>) => {
  const oldGames = await getOldResultBlackjackGames(1)
  ...
  for (const game of oldGames) {
    try {
      ...
      logger.error(`Blackjack idle close failed for game ${game.gameId}`, err)
    }
  }
  ...
  if (processed > 0) {
    ...
    for (const [guildId, count] of guildProcessed) {
      await postWorkerLog(client, {
        guildId,
        worker: 'Blackjack idle close',
        title: `Auto-closed ${count} idle table(s)`,
        description:
          'Settled blackjack tables with no new hand were closed after 24 hours.',
        level: 'info'
      })
    }
  }
