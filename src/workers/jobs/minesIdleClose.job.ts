...
export const minesIdleCloseJob = async (client: Client<true>) => {
  const oldGames = await getOldResultMinesGames(1)
  ...
  for (const game of oldGames) {
    try {
      ...
      logger.error(`Mines idle close failed for game ${game.gameId}`, err)
    }
  }
  ...
  if (processed > 0) {
    ...
    for (const [guildId, count] of guildProcessed) {
      await postWorkerLog(client, {
        guildId,
        worker: 'Mines idle close',
        title: `Auto-closed ${count} idle table(s)`,
        description:
          'Settled mines tables with no new board were closed after 24 hours.',
        level: 'info'
      })
    }
  }
