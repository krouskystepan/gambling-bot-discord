...
export async function run({ interaction }: SlashCommandProps) {
  try {
      ...
      if (vipRoom) {
        return interaction.reply({
          embeds: [
            createErrorEmbed(
              'VIP Already Active',
              `You already are Owner of VIP channel <#${vipRoom.channelId}>.`
            )
          ],
          flags: MessageFlags.Ephemeral
        })
    ...
    if (subcommand === 'add-member') {
        ...
        return interaction.reply({
          embeds: [
            createErrorEmbed(
              ...
              'The server administrator has disabled adding additional VIP members.'
            )
          ],
          flags: MessageFlags.Ephemeral
        })
      ...
      if (userToAdd.id === interaction.user.id) {
        return interaction.reply({
          embeds: [
            createInfoEmbed(
              'Not Allowed',
              'You are already the owner of this VIP room.'
            )
          ],
          flags: MessageFlags.Ephemeral
        })
      ...
      if (vipRoom.memberIds.includes(userToAdd.id)) {
        return interaction.reply({
          embeds: [
            createInfoEmbed(
              'Already Added',
              `${userToAdd} is already a member of your VIP room.`
            )
          ],
          flags: MessageFlags.Ephemeral
        })
      ...
      if (
        ...
        return interaction.reply({
          embeds: [
            createErrorEmbed(
              'VIP Full',
              `Your VIP room is full. Max members allowed: **${guildConfiguration.vipSettings.maxMembers}**`
            )
          ],
          flags: MessageFlags.Ephemeral
        })
      ...
      const additionalMemberPrice =
        guildConfiguration.vipSettings.pricePerAdditionalMember ?? 0
      ...
      if (user.balance < additionalMemberPrice) {
        return interaction.reply({
          embeds: [
            createErrorEmbed(
              'Insufficient Funds',
              `Adding a VIP member costs **$${formatNumberToReadableString(
                additionalMemberPrice
              )}**, but you only have **$${formatNumberToReadableString(user.balance)}**.`
            )
          ],
          flags: MessageFlags.Ephemeral
        })
      ...
      return interaction.editReply({
        embeds: [
          createSuccessEmbed(
            'Member Added',
            `${userToAdd} has been added to your VIP room.\n` +
      ...
      if (!vipRoom.memberIds.includes(userToRemove.id)) {
        return interaction.reply({
          embeds: [
            createInfoEmbed(
              'Not a Member',
              `${userToRemove} is not a member of your VIP room.`
            )
          ],
          flags: MessageFlags.Ephemeral
        })
